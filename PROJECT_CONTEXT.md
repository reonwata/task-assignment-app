...
#### スロット配分（人数決定）
- 優先順位: Case > SIM > Mail&C2C
- Mail&C2C: 出勤6名以下で1名、7名以上で2名固定
- 残りをCase → SIMの順で配分（Caseが同数かSIMより1名多い）
- タスクが一部のみ選択された場合は選択タスク間で均等配分

#### 割り当て対象者の決定
1. 各ラウンドで全未割り当てメンバーが「自分の中で一番回数が少ないタスク」を希望
2. 同じタスクに希望が集中した場合、そのタスクの回数が少ない人を優先（同数ならランダム）
3. あぶれた人は次のラウンドで残りスロットから再度希望を出す
4. 全員が割り当てられるまで繰り返す
...
| 5/18 | スロット配分を優先順位ベースに変更（Case>SIM>Mail、Mail最大2名） |
